  ...
  async onCron() {
    ...
    try {
      ...
      if (unstarredFiles.length > MAX_UNSTARRED_ITEMS) {
        console.log(`[${this.room.id}] Janitor: Found ${unstarredFiles.length} unstarred items, exceeding limit of ${MAX_UNSTARRED_ITEMS}. Cleaning up...`);
        ...
        console.log(`[${this.room.id}] Janitor: Successfully deleted ${deletedCount} old unstarred items.`);
        if (deletedCount > 0) {
        ...
        console.log(`[${this.room.id}] Janitor: Found ${unstarredFiles.length} unstarred items. No cleanup needed.`);
      }
    } catch (error) {
